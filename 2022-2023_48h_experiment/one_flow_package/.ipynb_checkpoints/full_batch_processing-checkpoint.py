...
for index, pair in enumerate(sar_pairs, start=1):  # start=1 to have human-friendly indexing
    ...
    output_dir_path = os.path.join(output_folder, f"{sar1.timestamp.strftime('%Y%m%dT%H%M%S')}_{sar2.timestamp.strftime('%Y%m%dT%H%M%S')}")
    try:
    ...
    if mod_dom is None:
        ...
        sar_drift_output_path = SAR1_SAR2_drift_retrivial.save_results_to_npz(output_folder, save_name,
    ...
    save_name = 'domain_output'
    sar_drift_output_path = SAR1_SAR2_drift_retrivial.save_results_to_npz(output_dir_path, save_name,
                                                                         X=X_subset, Y=Y_subset)
    ...
    upm, vpm, apm, rpm, hpm, ssim, lon2pm, lat2pm = SAR1_SAR2_drift_retrivial.combine_based_on_hessian(hh_hv_pm_plots_dir, folder_name, x, y, upm_hh, vpm_hh, apm_hh, rpm_hh, hpm_hh, ssim_hh, lon2pm_hh, lat2pm_hh, upm_hv, vpm_hv, apm_hv, rpm_hv, hpm_hv, ssim_hv, lon2pm_hv, lat2pm_hv)
    ...
    sar_disp_min, sar_disp_max = SAR1_SAR2_drift_retrivial.plot_filter_results(hh_hv_pm_plots_dir, x, y, hpm, upm, vpm, gpi1, gpi2, disp_legend_min, disp_legend_max, hessian, neighbors)
    ...
    SAR1_SAR2_drift_retrivial.save_results_to_npz(output_data_dir, save_name,
    ...
    s1_dst_dom_S_hv_masked = warping_with_domain.warp_with_uv(n1_hv, n1_hv[1], mod_dom, upm, vpm, mask_pm, dst_dom)
    ...
    s1_dst_dom_S_hv_masked0 = warping_with_domain.warp_with_uv(n1_hv, n1_hv[1], mod_dom, upm, vpm, mask_pm, dst_dom)
    ...
    warping_with_domain.plot_sar_forecast_images(warping_plots_save_dir, 
                                             ...
                                             s1_dst_dom_hv, s2_dst_dom_hv, s1_dst_dom_S_hv_masked,
                                             s1_dst_dom_hh, s2_dst_dom_hh, s1_dst_dom_S_hh_masked,
                                             gamma_value=1.2)
    warping_with_domain.plot_sar_forecast_images(warping_plots_save_dir, 
                                         "Forecast_with_sar_ref_drift_masked_0dist", 
                                         s1_dst_dom_hv, s2_dst_dom_hv, s1_dst_dom_S_hv_masked0,
                                         s1_dst_dom_hh, s2_dst_dom_hh, s1_dst_dom_S_hh_masked0,
                                         gamma_value=1.2)

    # 5.4. Save warped array to data output
    save_name = 'sar_warped_arrays'
    sar_distort_data_error_path = SAR1_SAR2_drift_retrivial.save_results_to_npz(output_data_dir, save_name, 
    ...
    # 5.5. Save SAR1 and SAR2 arras warped to dst domain
    ...
    mod_drift_output_path = SAR1_SAR2_drift_retrivial.save_results_to_npz(output_data_dir, save_name,
    ...
    mod_distort_data_error_path = SAR1_SAR2_drift_retrivial.save_results_to_npz(output_data_dir, save_name,
    ...
    upm_mod_comb, vpm_mod_comb, apm_mod_comb, rpm_mod_comb, hpm_mod_comb, ssim_mod_comb, lon2pm_mod_comb, lat2pm_mod_comb = SAR1_SAR2_drift_retrivial.combine_based_on_hessian(output_dir_path,mod_distort_plots_dir, x, y, upm_mod_hv, vpm_mod_hv, apm_mod_hv, rpm_mod_hv, hpm_mod_hv, ssim_mod_hv, lon2pm_mod_hv, lat2pm_mod_hv, upm_mod_ft_hv, vpm_mod_ft_hv, apm_mod_ft_hv, rpm_mod_ft_hv, hpm_mod_ft_hv, ssim_mod_ft_hv, lon2pm_mod_ft_hv, lat2pm_mod_ft_hv)
    ...
    gpi_mask_sar_warp = warping_with_domain.warp_and_mask_with_uv(mod_dom, gpi2, mod_dom, upm, vpm, mask_pm, mod_dom, max_dist=0, fill_value=0)
                                        ...
                                        (n1_hh, n1_hh[1], mod_dom, upm, vpm, mask_pm, dst_dom, max_dist=2, fill_value=0)
