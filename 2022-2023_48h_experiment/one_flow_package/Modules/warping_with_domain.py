...
def warp_and_mask_with_lonlat(src_dom, src_img, lon1, lat1, lon2, lat2, mask, warped_mask, dst_dom, max_dist=2, fill_value=0):
    ...
    dst_dist = warp_distance(dst_dom, lon1, lat1, warped_mask)
...
def warp_and_mask_with_uv(src_dom, src_img, uv_dom, u, v, mask, warped_mask,dst_dom, max_dist=2, fill_value=0):
    """ Warp input image on destination domain with drift compensation and masking if U,V,mask matrices are given """
    uv_srs = NSR(uv_dom.vrt.get_projection()[0])
    lon1uv, lat1uv = uv_dom.get_geolocation_grids()
    ...
    inp_img[:, 0] = 0
    inp_img[:, -1] = 0
    dst_img = warp_and_mask_with_lonlat(src_dom, inp_img, lon1uv, lat1uv, lon2uv, lat2uv, mask, warped_mask,dst_dom, max_dist=max_dist, fill_value=fill_value) #np.nan if want Nan
    return dst_img
...
#@profile
def plot_sar_forecast_images(general_save_path, file_name, s1_dst_dom_hv, s1_dst_dom_hh, s2_dst_dom_hv, s2_dst_dom_hh, s1_dst_dom_S_hv, s1_dst_dom_S_hh, gamma_value=1.2):
    """
    ...
    axs[2].set_title('SAR2 Predicted')
